...
//
// A sampler named `u_step_<name>` IS a draw. Its body is `step_<name>`, and the engine
// works out the running order from who reads whom: nothing is listed anywhere. Read a
// step's own sampler inside its body and you get LAST FRAME — that is feedback, with no
// second buffer to manage.
//
// Each step's target (size, format, filter) is set in the Steps panel, not here.
//
...
uniform sampler2D u_step_sparks;
uniform sampler2D u_step_bright;
...
void step_bright(out vec4 o) {
    vec3 c = texture(u_step_sparks, vs_uv).rgb;
...
void step_blur(out vec4 o) {
    vec2 t = 1.5 / vec2(textureSize(u_step_bright, 0));
    vec3 s = vec3(0.0);
    for (int x = -2; x <= 2; x++) {
        for (int y = -2; y <= 2; y++) {
            s += texture(u_step_bright, vs_uv + vec2(x, y) * t).rgb;
        }
    }
    o = vec4(s / 25.0, 1.0);
}

// --- step 4: FEEDBACK. Reading u_step_trail inside step_trail gives last frame. ----------
void step_trail(out vec4 o) {
    ...
    vec3 prev = texture(u_step_trail, vs_uv).rgb * u_fade;
    o = vec4(max(texture(u_step_sparks, vs_uv).rgb, prev), 1.0);
}
...
void main() {
    ...
    vec3 c = texture(u_step_trail, vs_uv).rgb
           + texture(u_step_sparks, vs_uv).rgb
           + texture(u_step_blur, vs_uv).rgb * u_bloom;
