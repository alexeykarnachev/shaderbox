...
def _get_file(file_id: str, output_path: Path) -> None:
    with httpx.Client() as client:
        response = client.get(
            f"{settings.modelbox_file_url}/{file_id}",
            timeout=10.0,
        )
        response.raise_for_status()
...
def infer_media_model(media: T, model_name: str, output_dir: Path) -> T:
    input_file_id = _post_file(media.details.file_details.path)
    ...
    output_file_id = response.json()["file_id"]
    output_file_path = Path(output_dir / output_file_id)
    _get_file(output_file_id, output_file_path)
