...
from shaderbox.core import Image, Node
from shaderbox.ui import VideoDetails
...
@post("/apply_node", media_type=MediaType.JSON)  # type: ignore
async def apply_node(data: ApplyNodeRequest = _body_multipart) -> ApplyNodeResult:
    try:
        ...
        file_path = str(_VIDEOS_DIR / f"{data.name}_{hash}.webm")
        ...
        node.render_video(
            VideoDetails(file_path=file_path, duration=2.0, fps=30),
        )
        logger.debug(f"Video rendered: {file_path}")
