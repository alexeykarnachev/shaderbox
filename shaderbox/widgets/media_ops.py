...
def draw_video_filters(app: App, input_video: Video) -> Video:
    ...
    with imgui_ctx.begin_group():
        ...
        document_ui_state.video_to_video_smoothing_window = imgui.drag_int(
            "##smoothing_window",
            document_ui_state.video_to_video_smoothing_window,
            v_min=3,
        )[1]

        row_label(app.font_12, "Sigma", float(SIZE.SMOOTHING_LABEL_W))
        imgui.set_next_item_width(SIZE.SMOOTHING_DRAG_W)
        document_ui_state.video_to_video_smoothing_sigma = imgui.drag_float(
            "##smoothing_sigma",
            document_ui_state.video_to_video_smoothing_sigma,
            v_min=0.01,
            v_speed=0.01,
        )[1]
