...
def draw_node_preview_button(
    ...
    return preview_cell(
        ...
        cell_w=size,
        texture_glo=ui_node.node.render_pass.canvas.texture.glo,
        texture_size=ui_node.node.render_pass.canvas.texture.size,
        selected=selected,
        armed=armed,
    ...
    with imgui_ctx.begin_child(
        ...
        for i, (id, ui_node) in enumerate(list(app.ui_nodes.items())):
            ...
            if id == app.current_node_id:
                if ui_node.node.render_pass.compile_unit.error_raw:
