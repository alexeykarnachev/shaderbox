...
# top. no_nav_focus keeps it out of imgui's Ctrl+Tab window-switcher (which the main
# window also suppresses), so our region-cycle keeps owning Ctrl+Tab.
#
...
_WINDOW_FLAGS = imgui.WindowFlags_.no_nav_focus
...
def _apply_layout(app: App) -> None:
    # CORNER / BOTTOM_STRIP force pos+size every frame (fixed presets). FREE seeds once
    # and lets imgui.ini persist the user's drag/resize.
    win_w, win_h = glfw.get_window_size(app.window)
    margin = float(SIZE.COPILOT_MARGIN)
    if app.copilot_layout == CopilotLayout.CORNER:
        w, h = float(SIZE.COPILOT_W), float(SIZE.COPILOT_H)
        imgui.set_next_window_pos((win_w - w - margin, win_h - h - margin))
        imgui.set_next_window_size((w, h))
    elif app.copilot_layout == CopilotLayout.BOTTOM_STRIP:
        h = float(SIZE.COPILOT_STRIP_H)
        imgui.set_next_window_pos((margin, win_h - h - margin))
        imgui.set_next_window_size((win_w - 2.0 * margin, h))
    else:
...
def draw(app: App) -> None:
    ...
    with imgui_ctx.begin("Copilot", flags=_WINDOW_FLAGS) as window:
        ...
        app.copilot_focused = imgui.is_window_focused(
            imgui.FocusedFlags_.child_windows
        )
