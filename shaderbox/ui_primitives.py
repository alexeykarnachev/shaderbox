...
def fps_chip(anchor_x: float, anchor_y: float, fps: int) -> bool:
    """The canvas's ONE chip: the current document's rate, and the opener for the canvas menu.

    `anchor_x` / `anchor_y` are the top-RIGHT screen corner of the region it hugs. Returns True
    on a click, which the caller turns into the menu (094 D13a).

    One number, not two. The chip used to read `58 | doc 30` -- the UI's rate beside the
    document's -- because the document's own rate is only defined while the throttle holds it
    below the frame rate. The caller resolves that instead (`_current_document_fps`), so what
    reaches here is always the rate the document is actually drawing at.
    """
    label = f"{fps} FPS"
    ...
    imgui.set_cursor_screen_pos((anchor_x - pill_w - inset, anchor_y + inset))
    return chip_button(label, pill_w, pill_h, faded=True)
