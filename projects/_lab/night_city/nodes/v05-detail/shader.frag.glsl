...
float sd_building(vec3 p, vec2 id, out float height) {
    ...
    height = h;
    float roof_y = GROUND_Y + 2.0 * h;                       // top of the box
    vec3 q = vec3(local_xz.x, p.y - (GROUND_Y + h), local_xz.y);
    float body = sdBox(q, vec3(BUILDING_HALF.x, h, BUILDING_HALF.y));

    // cheap rooftop clutter: a water-tank/penthouse box + a thin antenna mast, offset per building
    vec2 ro = (vec2(hash(vec3(id, 11.0)), hash(vec3(id, 12.0))) - 0.5) * BUILDING_HALF;
...
vec3 draw_facade(vec2 uv, vec2 idx, vec2 seed, vec2 bldg_seed, float n_floors) {
    ...
    float lit = hash(wkey) * 2.0;
    vec3 col = window_light_color(hash(wkey + 4.7)) * lit;
    bool has_curtains = hash(wkey + 99.9) > 0.3;

    vec3 wall = wall_color(uv, bldg_seed);

    // local position within this window cell, centered: [-0.5,0.5]^2
    vec2 q = vec2(window_uv.x - 0.5, window_uv.y - 0.5);
    vec2 hw = 0.5 * u_window_size;

    // --- recessed-window AO: darken a ring just OUTSIDE the glass so windows look inset ---
    float outside = max(abs(q.x) - hw.x, abs(q.y) - hw.y);     // >0 outside glass
    float ao = smoothstep(0.10, 0.0, outside) * smoothstep(-0.02, 0.04, outside);
    wall *= (1.0 - 0.55 * ao);

    // --- pilasters: a subtle raised column on the wall between window bays ---
    float pil = smoothstep(0.5, 0.42, abs(q.x));              // bright near the bay edges
    wall += vec3(0.010, 0.010, 0.011) * pil;

    // --- window sill: a brighter horizontal ledge just under the glass ---
    float sill = smoothstep(0.012, 0.0, abs(q.y - (hw.y + 0.03))) * step(abs(q.x), hw.x + 0.04);
    wall += vec3(0.05, 0.048, 0.045) * sill;

    bool is_bottom = idx.y < 0.5;                              // ground floor
    bool is_top = idx.y > n_floors - 1.5;                     // top floor band

    // --- plinth: a darker, window-less base at street level ---
    if (is_bottom) {
        wall = wall_color(uv, bldg_seed) * 0.55;
        wall += vec3(0.012) * smoothstep(0.5, 0.46, abs(q.x)); // base pilasters only
        return wall;                                           // no lit window on the plinth
    }
...
vec3 draw_sky(vec3 rd) {
    ...
    // --- aircraft crossing the sky: tiny emissive bodies with blinking strobe + R/G nav lights ---
    if (rd.y > 0.03) {
        vec2 suv = rd.xz / max(rd.y, 0.05);     // same sky-plane projection as the stars
        for (int a = 0; a < 2; ++a) {
            float fa = float(a);
            // sky-plane (suv) coords solved so the planes sit in the VISIBLE upper sky band
            // (suv.y ~ -2.5..-3.2 -> NDC y ~ 0.55..0.70 for this camera; see NOTES).
            float ylane = mix(-1.9, -2.2, fa);                  // above the skyline -> clear sky
            float speed = mix(0.22, 0.16, fa);
            float xpos = mod(u_time * speed + fa * 1.9, 3.2) - 1.9; // sweep across the open band, wrap
            vec2 ppos = vec2(xpos, ylane);
            vec2 dlt = suv - ppos;
            // fuselage: a small steady dim body so the plane never fully vanishes between strobes
            float body = smoothstep(0.10, 0.0, length(dlt * vec2(0.7, 1.6)));
            col += vec3(0.55, 0.56, 0.62) * body * 0.6;
            // white anti-collision strobe (fast blink) — bright when on
            float strobe = step(0.86, fract(u_time * 1.5 + fa * 0.5));
            col += vec3(1.2) * smoothstep(0.07, 0.0, length(dlt)) * strobe;
            // steady wingtip nav lights: red on left wing, green on right (offsets sized for suv space)
            float red = smoothstep(0.05, 0.0, length(dlt - vec2(-0.11, 0.0)));
            float grn = smoothstep(0.05, 0.0, length(dlt - vec2(0.11, 0.0)));
            col += vec3(1.0, 0.1, 0.05) * red * 1.1;
            col += vec3(0.1, 1.0, 0.2) * grn * 1.1;
        }
    }
    return col;
}
...
void main() {
    ...
    if (is_building && is_vertical_face) {
        ...
        float n_floors = floor((2.0 * rm.height) / FLOOR_H);
