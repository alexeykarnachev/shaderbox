...
float smoke_density(vec3 q, float t) {
    // Time-scroll the NOISE LOOKUP only (puffs rise + drift through depth). The
    // placement envelope below reads the UNSCROLLED q, so the smoke REGION stays
    // put while its texture flows up. (Earlier bug: scrolling q itself dragged the
    // whole band off-frame, so smoke vanished as time advanced.)
    vec3 nq = q + vec3(0.0, -t * 0.5, t * 0.35);
    float f = 0.0, amp = 0.5, freq = 1.0;
    for (int i = 0; i < 5; i++) {
        f += amp * noise3(nq * freq);
        freq *= 2.05;
        amp *= 0.5;
    }
    ...
    return clamp(f * col * band - 0.18, 0.0, 1.0); // -0.18 erodes into ragged wisps
}
...
void main() {
    ...
    float heat = flame_heat(p, u_time, n);   // flame body stays steady

    // Flicker drives the AMBIENT light it casts, not the flame colour. A faint
    // breath stays on the body so it isn't dead, but the SPREAD is in the glow.
