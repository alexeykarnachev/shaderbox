...
uniform float u_rm_max_n_steps = 256;
...
vec3 sph_to_cart(float theta, float phi) {
...
ParallaxResult do_parallax(vec2 uv, vec3 cam_pos, float max_n_steps) {
    vec3 sp = vec3((uv - 0.5) * vec2(u_aspect, 1.0), 0.0);

    ParallaxResult result;
    result.p = sp;
    result.ray_dir = normalize(sp - cam_pos);
    result.step_idx = 0.0;
    result.step_size = 1.0 / max_n_steps;
    result.is_success = 0.0;

    while (true) {
        if (result.step_idx++ >= max_n_steps) {
            break;
        }

        result.uv = result.p.xy / vec2(u_aspect, 1.0) + 0.5;
        if (result.uv.x >= 1.0 || result.uv.x <= 0.0 || result.uv.y >= 1.0 ||
            result.uv.y <= 0.0) {
            break;
        }

        result.depth = clamp(texture2D(u_depth, result.uv).r, 0.0, 1.0);
        result.diff = result.p.z - result.depth;
        ...
        result.p += result.step_size * result.ray_dir;
    }
