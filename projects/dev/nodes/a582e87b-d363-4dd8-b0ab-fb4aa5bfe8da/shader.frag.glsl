...
vec3 animated_color(float t) {
    // simple hue cycle, no library
    float h = 0.15 * t;
    return 0.5 + 0.5 * vec3(sin(h), sin(h + 2.0), sin(h + 4.0));
}
...
void main() {
      ...
      for (uint i = 0; i < MAX_TEXT_LEN; ++i) {
          ...
          // simple wavy animation on letters
          float wave = 0.2 * sin(u_time * 4.0 + float(col) * 1.3);
          vec2 char_pos = vec2(col * (1.0 + u_text_spacing.x),
                               u_zoomout - row * line_step + wave);

          float scale = u_char_scale * (0.95 + 0.05 * sin(u_time * 3.0));
          vec2 p = 2.0 * (uv - char_pos) / scale;
      ...
      vec3 base = animated_color(u_time);
      vec3 color = base * line;
