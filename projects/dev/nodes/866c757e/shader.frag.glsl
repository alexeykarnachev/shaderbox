...
void main() {
    ...
    while (true) {
        ...
        if (diff >= 0.0) {
            ...
            video_color = pow(video_color, vec3(1.5));
            vec3 photo_color = texture2D(u_photo, uv).rgb;
            float p = snoise(vec4(4.0 * uv, 0.0, 0.0));
            p = (p + 1.0) / 2.0;
            p = smoothstep(0.5, 0.7, p);
            color = mix(video_color, photo_color, p);
            break;
        }
    ...
    float alpha = 1.0;
    if (color.r + color.g + color.b < 0.02) {
        alpha = 0.0;
    }
