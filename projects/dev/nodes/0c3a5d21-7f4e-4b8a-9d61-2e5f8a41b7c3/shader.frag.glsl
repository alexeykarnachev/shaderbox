...
void step_blur(out vec4 o) {
    ...
    vec2 t = 1.5 / vec2(textureSize(u_step_scene, 0));
    vec4 s = texture(u_step_scene, vs_uv + vec2( t.x,  t.y))
           + texture(u_step_scene, vs_uv + vec2(-t.x,  t.y))
           + texture(u_step_scene, vs_uv + vec2( t.x, -t.y))
           + texture(u_step_scene, vs_uv + vec2(-t.x, -t.y));
    o = s * 0.25;
}
...
void step_trail(out vec4 o) {
    ...
    o = max(texture(u_step_blur, vs_uv), prev);
}

void main() {
    vec3 c = texture(u_step_scene, vs_uv).rgb + texture(u_step_trail, vs_uv).rgb;
