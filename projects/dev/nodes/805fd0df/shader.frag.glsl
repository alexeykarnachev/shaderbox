...
uniform vec3 u_global_0_color = vec3(1.0, 0.0, 0.0);
uniform vec3 u_global_1_color = vec3(0.0, 1.0, 0.0);
...
void main() {
    ...
    vec2 offset = vec2(snoise(vec4(500.0 * initial_uv, 0.0, progress)),
                       snoise(vec4(progress, 0.0, 500.0 * initial_uv)));
    offset = 0.0025 * offset * mod(u_time, u_period * 0.25) / (0.25 * u_period);
    initial_uv += offset;
    ...
    vec3 global_color = mix(u_global_0_color, u_global_1_color, progress);
    vec3 scene_color = (1.0 - depth) * global_color;
