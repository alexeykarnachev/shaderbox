...
uniform vec3 u_cam_pos = vec3(0.0, 5.0, 5.0);
uniform vec3 u_cam_look_at = vec3(0.0, 0.0, 0.0);
...
float get_sd_shape(vec3 point) {
    ...
    float left_pilar_d;
    {
        vec3 p = point;
        p.x += 0.9;
        float w = 0.18 + 0.05 * p.y;
        float h = 4.0;
        float f = 0.8;
        float a = 0.25;
        float d = length(vec3(p.xz + vec2(a * sin(f * p.y), 0.0),
                              min(max(p.y - h, 0.0), p.y))) -
                  w;
        left_pilar_d = d;
    }
    ...
    float top_pilar_d;
    {
        ...
        float d = length(vec3(p.yz + vec2(a * sin(-f * p.x), 0.0),
                              min(max(p.x - h, 0.0), p.x))) -
                  w;

        top_pilar_d = d;
    }

    float d = min(left_pilar_d, right_pilar_d);
    d = min(d, top_pilar_d);
    return d;
}
...
RayMarchResult march(vec3 ro, vec3 rd) {
    ...
    if (rm.sd_last < RM_EPS) {
        ...
        if (rm.sd_last == rm.sd_min_shape) {
            ...
            rm.n = normalize(
                vec3(get_sd_shape(rm.p + e.xyy) - get_sd_shape(rm.p - e.xyy),
