...
emitted GLSL is constant arrays + one small evaluator loop. This replaces the old
...
Same SDF semantics, same lattice, pixel-identical strokes.

Usage: uv run python scripts/gen_glyphs.py [out_path]
Default out: shaderbox/resources/shader_lib/text/glyphs.glsl
"""
...
def _stroke_payload(st: Stroke) -> str:
    kind = st[0]
    if kind == "seg":
        (ax, ay), (bx, by) = st[1], st[2]
        return f"vec4({_f(ax)}, {_f(ay)}, {_f(bx)}, {_f(by)})"
    if kind in ("arc", "tall"):
        (cx, cy), sx, sy = st[1], st[2], st[3]
        return f"vec4({_f(cx)}, {_f(cy)}, {_f(float(sx))}, {_f(float(sy))})"
    (cx, cy), r = st[1], st[2]
    return f"vec4({_f(cx)}, {_f(cy)}, {_f(r)}, 0.0)"
...
def generate() -> str:
    ...
    return f"""// Segment-glyph face, DATA-DRIVEN: glyph geometry lives in constant tables and
