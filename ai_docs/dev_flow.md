...
`projects/_lab/`; when a run yields something worth keeping — a promoted document dir (`git add -f`) or
...
- **`document.py`** — `Document`: what you open, save and export. Owns `passes` (name → `Pass`) plus
...
- **`render_preset.py`** — GL-free `RenderPreset` (an outlet's render constraints: size/aspect/fps/
  duration/container/byte-cap/fit) + `resolve_dims`. Drives `document.py::render_media`. Feature 010.
- **`shader_lib/`** — the auto-resolved GLSL helper library (features 015/016/017; the CONTENT —
  the SDF-layered seed — is feature 032: canonical copy at `shaderbox/resources/shader_lib/`,
  glyph tables generated by `scripts/gen_glyphs.py`, live root at `app_data_dir()/shader_lib`,
  seeded and kept in sync at startup by `shader_lib/seed.py`). Import the public
...
- **`copilot/`** — the in-app coding-copilot agent (feature 020, cross-project + gate-UI + render/publish +
  ...
  `tools/shader.py` — read_shader / edit_shader / write_shader / set_uniform / create_document / grep /
  read_lib / delete_document / switch_document; `tools/script.py` — read_script / write_script / edit_script;
  ...
  Yes/No (`delete_document` + publish + pack mutations) and CREDENTIAL, a masked secret input
  ...
  `backend.py` (serialize the LIVE document, never the on-disk dir) + `RevertExecutor.restore_checkpoint`
...
- **`dogfood/`** — the dogfooding STATION (feature 075): `report/log.py` (the append-only JSONL
  ...
  site; `uv run python -m dogfood.report.build [--watch]`), `runs/<experiment>/` (the local,
  gitignored record: `events.jsonl` + the renders + the pages), `index.html` (the bookmark; generated). It
...
object (e.g. a `Document`, call `render()` once to compile lazily, then assert on
...
### Authoring / debugging documents directly (from Claude Code, no app/copilot)
A document is just files on disk — `<project>/documents/<uuid>/{document.json, graph.json,
...
a document directly for ad-hoc shader work, a repro, or a `/shader-lab` session — no `App`, no copilot. The
mechanics that avoid the foot-guns hit in practice:

- **Write a document dir ATOMICALLY: the pass files BEFORE `document.json`** (or temp-dir + rename). A
  running app reconciles `documents/` every frame (`sync_documents_from_disk`); a dir with `document.json` but no
  pass file is a half-written document the sync tries to load. The sync skips an incomplete dir
  (document.json plus at least one pass file required), but writing passes-first closes the window
  regardless of the user's build. `graph.json` is optional — a pass file with no entry gets defaults,
  so a hand-authored single-pass document needs only `passes/main.frag.glsl` + `document.json`.
...
- **Render a document headless to eyeball it** (the loop the copilot can't run — it's render-blind, you
  ...
  ```python
  doc, meta = Document.load_from_dir(dir, ctx)   # returns (document, metadata)
  doc.set_canvas_size((w, h))                    # never render_pass.canvas.set_size — see below
  for frame in range(n):
      doc.begin_frame(frame)                     # advance feedback ONCE per frame
      doc.render(u_time=frame / fps)
  Image.fromarray(texture_to_rgba8(doc.render_pass.canvas.texture), "RGBA").save(path)
  ```

  `texture_to_rgba8`, never `texture.read()`: a pass target is `f2` by default and a raw read gives
  half of one float16 channel (`conventions.md ## Known quirks`). (`/shader-lab` ships a ready
  `render_document.py` for the PNG/MP4 case; note it forces a SQUARE canvas — for a non-square
  aspect, set the real size yourself.)
- **Check the shaders actually compiled** via `[n for n, p in doc.passes.items() if p.program is None]`
  plus `doc.graph_errors` (wiring, not source) after a `render()` — do NOT grep stderr for "error":
  the harmless `GLFWError` import-probe warning matches it and yields false fails.
...
failure**, so a pyright error cannot reach a commit through the gate.
...
tmp project (seeded with the shipped example documents + one script-driven-uniform document so the feature-040
engine ticks in the loop; never `projects/dev/`) in an invisible glfw window, asserts popup-mutex +
`current_document_id` invariants. ~1.5s; catches import errors, callback dispatch
