...
- **`copilot/`** — the in-app coding-copilot agent (feature 020, cross-project tools + the gate-UI wave
  DONE; render/publish + UI/UX polish are future waves). Mirrors `exporters/`: its own package + worker
  thread + queues + a worker→main GL `bridge`. `App` owns a `CopilotSession` handle + drains it per frame;
  the chat is a floating window (`widgets/copilot_chat.py`) launched from the editor bottom bar. The seams
  (`capabilities` Protocol / `llm.api` / `bridge` / queues / `state`), the `agent` loop, the `prompt`, the
  `trace`, and 9 eager tools (`tools/shader.py` — read_shader / the 3 edit tools / set_uniform / create_node
  / grep / read_lib / delete_node) are all built. The node-id/edit/uniform/delete machinery lives on `App`
  as `_copilot_*` closures bound into `CopilotCapabilities`. `gate.py` (`GateChannel`) is the worker→UI
  blocking confirm round-trip (the bridge's mirror) — its CONFIRM body is wired (used by `delete_node`); the
  CREDENTIAL kind stays a type-only stub for the publish-tools wave. Full design:
  `ai_docs/features/020_copilot_agent/` (`17_gate_ui.md` is the current spec; `10_skeleton_plan.md` for the
  original structure).
