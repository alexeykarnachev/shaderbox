...
- **Shipped: `v0.12.1`** (feature 019). Live on itch.io. Unshipped commits on `dev` (020 copilot wave +
  021 logging + 020·12 edit-robustness + 020·13 lexer + 020·14 line-editing) — `master` NOT advanced; do
  not ship without an explicit ask.
- **020 copilot agent — Slice 1 + the full edit toolset DONE.** The edit/compile-feedback vertical works
  end-to-end (default model `x-ai/grok-4-fast`); slice-12 self-correction (retry cap, chat cutoffs,
  near-miss hint) maintainer-verified live. The maintainer's 2-step sequence is COMPLETE: **020·13 lexer**
  (`copilot/glsl_lex.py` — `edit_shader` matches by GLSL token-stream equality, whitespace-invariant) +
  **020·14 Slice 2** (two line-anchored tools `replace_lines`/`insert_after` addressed by the line numbers
  `get_current_shader` shows + a "what changed" apply-feedback excerpt on every mutating edit + the
  retry-cap widened to all mutating tools). Both pre+post reviewed to convergence. 136 tests green; lexer
  maintainer-verified live, Slice 2 NEEDS a live maintainer drive ("add a uniform and use it" — confirm
  the model uses the line tools + the excerpt shows).
...
- **NEXT — Feature 022, copilot chat persistence (SPECCED, not started).** Conversation tied to its
  project + restored on reopen + clear-chat button; `project_dir/copilot/conversation.json` (versioned,
  fail-soft like `app_state.json`). Folds in the deferred copilot-trace cross-project bleed (it needs the
  same worker-quiesce-at-switch point). Spec: `022_copilot_chat_persistence.md`.
- **Also queued (trace-gated, not scheduled):** a copilot semantic-editing suite (rename/outline/
