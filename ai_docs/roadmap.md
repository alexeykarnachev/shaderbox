...
- **020·16 cross-project tools — DONE + LIVE-VERIFIED across 3 maintainer test sessions.** The agent works
  the whole project (8 eager tools `read_shader`/`edit_shader`/`replace_lines`/`insert_after`/`set_uniform`/
  `create_node`/`grep`/`read_lib`, an always-in-prompt project map + lib catalogue, per-node freshness
  keying, live-source lib creation). Three post-test fixes landed + re-verified: `create_node` returns its
  compile result (no more silent success on broken shaders); a GLSL-skeleton + engine-uniform-declaration
  note in the conventions (the agent now writes compiling shaders first try); short 4-char node ids in the
  prompt/tools (no UUIDs in chat; agent refers to nodes by name). Spec: `16_cross_project_tools.md`.
- **NEXT — pick the next capability wave (backlog recalled, decision pending).** Leading candidate: the
  **gate-UI wave** — build the `GateChannel` UI body (stubbed: the loop never checks `requires_gate`,
  `take_pending`/`answer` have no caller) → unblocks **`delete_node`** (the live trigger: a test user asked
  "remove the last 3") + `delete_lib_file`. Then Tier 2: render/publish tools (`render_image`/`render_video`,
  telegram/youtube wrappers — always-gate, 3s-cap-in-schema). Full backlog tiers in `16 ## Out of scope`
  + `11 §13`.
