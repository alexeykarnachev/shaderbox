...
- **DO NOT SHIP YET.** The copilot is NOT ship-ready (maintainer, explicit). Before any ship: the **copilot
  UI/UX polish wave** (still to be planned — the ship gate). `master` stays at `v0.12.1`; a large unshipped
  stack sits on `dev` (020 wave 12→19 + 021 logging + 022 persistence) — intentional, keep accumulating.
- **020·18 render/publish — DONE, maintainer-tested live (a real sticker uploaded end-to-end).** 4 always-
  gated tools: `render_image`/`render_video` → the project `renders/` dir (GL via the bridge,
  `FIXED_DIMS` so the requested size drives the output); `publish_telegram`/`publish_youtube` render-then-
  enqueue-then-await the exporter's terminal progress through the bridge. Spec: `18_render_publish_tools.md`.
- **020·19 Telegram connect/pack wave — DONE + headless-verified; live LLM/connect path awaits a maintainer
  pass.** The friction the live 020·18 test exposed (the agent could only send the user to Settings) is
  wired away: `set_telegram_token` opens a **masked inline secret input** (the `GateKind.CREDENTIAL` widget,
  built this wave — the token is redacted to a prefix everywhere but the live store: never in LLM history,
  the persisted conversation, the trace, or any log; H4 in `copilot_render_check.py` pins it) + auto-link
  (the `AuthState.LINKING` floor + a connect-await; the user must press Start on the bot once — Telegram
  rule) + full pack CRUD (list/select/create/delete, gated). Spec: `19_credential_pack_tools.md` (3 OPEN
  QUESTIONS: redaction length=6; the unavoidable two-step connect; delete = a real Telegram delete).
- **NEXT — the copilot UI/UX polish wave (to be planned).** It's the gate to shipping the whole 020 stack.
  Deferred siblings, all trigger-gated in `todo.md`: the "Rendering…" modal, the lazy tool-catalogue
  (its ~16-tool eager-count threshold has now FIRED), `delete_lib_file`, semantic-editing.
