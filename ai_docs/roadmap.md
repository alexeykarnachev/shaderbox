...
- **NEXT — Tier 2 render/publish tools, OR the copilot UI/UX polish wave.** Render/publish: `render_image`/
  ...
  `delete_lib_file` is a small sibling of `delete_node` (same gate path), deferred — no live trigger yet.
  Full backlog tiers in `16 ## Out of scope` + `11 §13` + `17 ## Out of scope`.
