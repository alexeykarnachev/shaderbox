...
- **DO NOT SHIP YET.** The copilot is NOT ship-ready (maintainer, explicit). Before any ship: the **copilot
  ...
  stack sits on `dev` (020 wave 12→18 + 021 logging + 022 persistence) — intentional, keep accumulating.
- **020·18 render/publish wave — DONE + headless-verified; LLM/upload paths await a maintainer live pass.**
  Four always-gated tools: `render_image`/`render_video` write to the project `renders/` dir (GL via the
  bridge with `render_op_timeout_s`, `FIXED_DIMS` so the requested size actually drives the output);
  `publish_telegram`/`publish_youtube` render-then-enqueue-then-await the exporter's terminal progress, ALL
  marshalled through the bridge (the frame loop stays responsive during the network upload). A new pre-gate
  `precheck` seam on `ToolDefinition` gives a guided handoff (connect in Settings / pick a pack / open a
  node) BEFORE the gate fires — missing creds never pop a doomed confirm. The exporters gained public
  `is_connected()`/`publish()` (the dead `export()` stub repurposed). Headless H1/H2/H3 in
  `scripts/copilot_render_check.py` (registry builds 13 tools, render honors the requested size, prechecks
  hand off). 4 OPEN QUESTIONS for the maintainer in `18_render_publish_tools.md` (eager-not-lazy publish;
  guided-handoff vs inline CREDENTIAL widget; default-pack-only; no "Rendering…" modal — the last reverses
  the dated R3 decision, needs re-confirm). Spec: `18_render_publish_tools.md`.
- **NEXT — the copilot UI/UX polish wave (to be planned).** It's the gate to shipping the whole 020 stack.
  ...
  (`search_tools`/`list_tools`), the inline CREDENTIAL widget, `delete_lib_file`, Telegram pack CRUD.
