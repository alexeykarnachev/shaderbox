...
- **A leader binding's id crossing the ABI is its row INDEX in `LEADER_BINDINGS`, and the drain
  ...
  **id 0 is both valid and falsy**, so the drain tests `is None`, never truthiness. A
  ...
  test fails the moment one is not. **Every one-slot ABI queue drains per key, not per frame**
...
- **The vendored editor binary (`shaderbox/resources/editor/`) rebuilds from a COMMITTED editor-repo
  ...
  platform-conditional code, no C links) but **Odin cannot cross-LINK from linux** ("Linking for
  cross compilation ... not yet supported", compiler-level) — so the `.dll` is built ON a Windows
  host with the same command, `-out:libeditor.dll`, at ship time, alongside the runtime verify
  BUILDING.md already requires there. Two measured facts the
