...
> **Status: SPEC (plan-locked 2026-06-15).** Design locked to Option C below (maintainer call after a
> brainstorm + a voice-note review). Supersedes the earlier DRAFT, which framed this narrowly as
> "reopen the closed shader tab" — that is one *symptom* of the real gap (see ## Goal). Implement from
> the locked Design decisions; the only open item is wording polish, flagged in ## Open questions.
