...
`read_shaders` (`backend.py::read_shaders`) reads `document.render_pass` — the OUTPUT pass only. The
...
(`backend.py::probe_render`'s reject is the shape to copy). A project pass-read joins the working set as its
DOCUMENT, mirroring `read_working_set` (`backend.py::read_working_set`, D11); only the example path returns
per-pass listings.

**D2 — the pass-address contract gets one registry-enumerated gate.** `c8960e1` fixed
`probe_render` and wrote into `tests/test_copilot_pass_tools.py` that "every other tool
...
error, and silently downgrading it to the document is what `conventions.md ## Design decisions` forbids
("NEVER change a destructive edit's behavior on a heuristic GUESS the model can't see"). The shared
root is the source-read seam, matching `conventions.md ## Design decisions` — the convention 065 did not apply.
...
and `sanitize.py`'s glyph table maps that character only on the commit path into history — em-dash form in
replies 0, ASCII form 3. The models copied HISTORY'S RENDERING, so `_render_summary`
(`session.py::_render_summary`), which joins model prose and engine ledger into one unmarked assistant
message, is the learning channel. No guard can see this shape: `agent.py`'s `silent_after_tool`
...
reply falls through `agent.py`'s no-tool-call branch to the ordinary success terminal at `agent.py`'s turn-done terminal. The fix
...
turns, and has no enum to mutation-test against. And `prompt.py`'s claiming-an-action rule already IS the honesty rule;
...
turn because `loaded_tools` is a `run_turn` local (`agent.py`'s `loaded_tools`), and 26 of 37 `load_tools` calls
...
**D7 — the batching instruction is re-justified by COST, not by steps.** `prompt.py`'s BATCH rule sells
...
`TurnStats.context_tokens` is fed `first_input_tokens` (`agent.py`'s `first_input_tokens`, "iter-0 context size for
the usage bar") and `context_gauge_readout` (`state.py::context_gauge_readout`) prints it beside `cost_usd`, which
...
**D10 — `PassView` carries reachability.** `widgets/pass_list.py`'s live set already computes the live
set via `evaluation_order` and dims unreachable tiles for the human; `PassView`
(`capabilities.py::PassView`) carries `name, address, listing, uniforms, errors, is_output` and no
...
**D11 — `_validation_message` names the offending field.** `tools/registry.py::_validation_message` reads `msg`
...
(`tools/shader.py::_SET_UNIFORM_DESC`) and `_CONVENTIONS` (`prompt_context.py::_CONVENTIONS`) name three of the five
...
(`shaderbox/engine_uniforms.py::ENGINE_UNIFORM_TYPES`), stating the positive branch: to change what a pass counter does, edit
...
`build.py`'s warn span) instead of an absent section, since today zero real commits and three unswept
commits render identically. The same row surfaces `dirty`, which `station.py::start_attempt` records on every
attempt and `Attempt` (`log.py::Attempt`) has no field for — a write-only integrity signal that
fired on all 8 `rc_full_build` attempts, meaning those runs did not execute the code their sha
names.

**D14 — the estimator constant is measured.** `_CHARS_PER_TOKEN = 4` (`prompt.py::CHARS_PER_TOKEN`) is duplicated
as a bare `// 4` (`context_breakdown.py`'s tools estimate) and has zero tests. Measured implied divisor across
...
the driver calls BY HAND is the sanctioned shape; `conventions.md ## Design decisions` is a settled decision
...
- **The turn-boundary cache miss** (ceiling 4.3% of input, less in dollars). The alternative —
  ...
  `dialogue` block re-paid forever and reintroduces the stale-source hazard `session.py`'s NL-only history note
...
- **Cutting static prompt prose.** Only 20.6% of floor tokens are ever billed fresh, so every prose
  deletion recovers ~1% of a run. VISUAL CRAFT is the section paying for itself: `prompt.py`'s VISUAL CRAFT block
...
- **The seven never-called eager tools** (~1,548 tok/request, ~723k est tokens = 7% of billed
  ...
  `switch_document` is prompt-critical (`prompt.py`'s publish rule) regardless of its cold record.
...
| D2 | Revert `probe_render`'s split (`backend.py::probe_render`); the gate names `probe_render`. Then add a throwaway tool with a `document` field and no classification; the gate fails naming it. |
| D4 | A fake client yielding text + `LLMDone("stop")` with no tool calls (the `_PlainClient` shape, `tests/test_copilot_loop.py`'s `_PlainClient`): the turn must not end as a bare `AgentTurnDone`. Delete the branch and watch it fail. Inverse: one real call plus the same prose still ends in ONE stream, or the gate has widened its domain to all turns. |
...
performed and named in its commit at the time.

**Then `fcbdf06` deleted six of the eleven gates as ceremony, and an audit found that call half
wrong.** Two of the six were restored: the `probe_render` pass-address test (which the wave had
absorbed from a pre-existing regression test for `c8960e1`, so deleting it left a shipped fix
unguarded) and the render pair's named refusal. The other four stay deleted — a boolean on a tool
definition, a constant's band, a pure four-line function, and one whose subject a surviving test
already covers. D12, D14 and D15 are therefore live and ungated by choice; D2's class claim rests
on the render-refusal test plus `read_shader`'s own.

Four corrections the implementation forced:
...
**A later audit found three code defects, all fixed:** `billed_input_tokens` was not round-tripped
by `persistence.py`, so the gauge read 0 after a project reload; the `missing` note compared whole
handles, so an abbreviated pass address (`537#red` for `5372#red`) read successfully and was then
reported as not found; and a bad pass name raised from inside `read_shaders`' loop, discarding
every handle that had resolved in the same call.
