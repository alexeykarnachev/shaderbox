...
Status: **done.** Implemented in two waves, reviewed to closure (four post-implementation rounds; each angle's final round a PASS: architecture in round 3, spec fidelity in round 2 on the code, correctness in round 4), gates green with the smoke run; the maintainer's walk of the canvas is 093 (the manual list below was his checklist). Sketches: `00_mock.html` (round 3 is the
