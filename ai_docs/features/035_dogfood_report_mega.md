...
- #1: the giveup loop -- FIXED post-run (the 035 fix wave, forensic-swarm-driven). Real roots: the lib indexer block-comment misalignment (strip_comments vs raw_lines -- one-line fix, bug present since v0.12.1), edit errors not naming the resolved target, boundary rejects not flagging multi-line quotes nor locating the quote, the giveup note hiding applied work. Counter design itself reviewed and kept (reset-on-applied-edit already existed and worked; the one deferred tweak: read tools should not reset the counter).
