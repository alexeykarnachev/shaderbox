...
- **Never leave `projects/dev/` unstaged** — sandbox drift gets `git add projects/dev && commit`ed in the same wave; never `checkout`-ed away. It never reaches a shipped bundle: `build.sh` copies an allowlist that has no `projects/` in it. (Full rule: `dev_flow.md`.)
