...
snapshot copy must never leave the live document dir destroyed, and restore_checkpoint
contains a per-document failure as `unrestorable` instead of raising."""
...
def test_swap_replaces_live_dir_with_snapshot(tmp_path: Path) -> None:
    snap = tmp_path / "snap"
    snap.mkdir()
    (snap / "document.json").write_text('{"from": "snapshot"}', encoding="utf-8")
    dst = tmp_path / "documents" / "abc"
    dst.mkdir(parents=True)
    (dst / "document.json").write_text('{"from": "live"}', encoding="utf-8")
    (dst / "stale.txt").write_text("x", encoding="utf-8")

    _swap_in_snapshot(snap, dst)

    assert (dst / "document.json").read_text(encoding="utf-8") == '{"from": "snapshot"}'
...
def test_failed_copy_leaves_live_dir_intact(tmp_path: Path) -> None:
    snap = tmp_path / "snap"
    snap.mkdir()
    (snap / "document.json").write_text("{}", encoding="utf-8")
    ...
    assert (dst / "document.json").read_text(encoding="utf-8") == '{"from": "live"}'
    assert not dst.with_name("abc.restoring").exists()
...
def test_restore_checkpoint_contains_a_failed_document_restore(tmp_path: Path) -> None:
    ...
    active.snapshotted_documents["abc"] = "MyDocument"
    store.seal()

    documents_dir = tmp_path / "documents"
    live = documents_dir / "abc"
    live.mkdir(parents=True)
    (live / "document.json").write_text('{"from": "live"}', encoding="utf-8")

    executor = RevertExecutor(
        get_documents_dir=lambda: documents_dir,
        get_trash_dir=lambda: tmp_path / "trash",
        get_ui_documents=lambda: {},
        get_checkpoints=lambda: store,
        get_shader_lib_files=_unused_lib_files,
        set_current_document_id=lambda _document_id: None,
        sync_editor_from_disk=lambda _document_id, _text: None,
        delete_document_unguarded=lambda _document_id: "",
        invalidate_lib_consumers=lambda _path: None,
    )

    result = executor.restore_checkpoint("turn_x")  # must NOT raise

    assert result.unrestorable == ["MyDocument"]
    assert result.restored_documents == []
    assert (live / "document.json").read_text(encoding="utf-8") == '{"from": "live"}'
