...
def test_an_exported_frame_carries_the_scripted_value(app: Any, tmp_path: Path) -> None:
    ...
    node.render_pass.release_program(_SRC)
    node.render_pass.source.path.write_text(_SRC)
    node.render_pass.compile()
    assert node.render_pass.compile_unit.errors == []

    scripts_dir = app.session.paths.scripts_dir_for(node_id)
    scripts_dir.mkdir(parents=True, exist_ok=True)
    (scripts_dir / NODE_SCRIPT_BASENAME).write_text(_SCRIPT)
    app.session.reload_scripts()

    # Warm the live instance well past what one cold export tick can reach.
    for frame in range(400):
        app.session.tick([node_id], frame / 60, 1 / 60, frame)
    live_level = node.render_pass.uniform_values["u_level"]
    ...
    width, height = node.render_pass.canvas.texture.size
