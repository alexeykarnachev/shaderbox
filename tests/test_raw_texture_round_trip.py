...
The branch had demonstrably never executed. Three defects sat in it at once: `UINode.save`
wrote `textures/<name>.bin` while only the node dir was mkdir'd (`FileNotFoundError` on first
contact), it recorded no `dtype`, and `Node.load_from_dir` passed none to `gl.texture(...)` --
...
_EXAMPLE = (
    ...
    / "shaderbox/resources/node_examples/73ea2431-13f6-41e4-b923-04d846b678b0"
)
_SAMPLER = "u_image"
