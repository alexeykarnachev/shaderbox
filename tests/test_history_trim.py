...
TURNS once the request estimate exceeds max_input_tokens, always keeping the last COPILOT_ENGINE.history_min_kept_turns and
...
def test_over_budget_drops_leading_turns_keeps_min() -> None:
    ...
    assert len(kept_turns) == COPILOT_ENGINE.history_min_kept_turns
    # The KEPT turns are the most RECENT ones (oldest dropped from the front).
    assert kept_turns[-1][0].content == "u19"
    assert kept_turns[0][0].content == f"u{20 - COPILOT_ENGINE.history_min_kept_turns}"
