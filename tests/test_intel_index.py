...
def test_declarations_are_what_the_buffer_lacks() -> None:
    ...
    # A script uniform with no literal shape is a name, never a guessed declaration. The type
    # comes from the live VALUE instead (`tabs/code.py::_typed_script_returns`), so by the time a
    # real script's key reaches here it usually carries one.
